...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> loadModel() async {
    Tflite.close();
    try {
      String modelFile = 'assets/Soil.tflite';
      String labelsFile = 'assets/Soil_labels.txt';
  ...
  Future<List<String>> getPrecautions(String label) async {
    String csvString = await rootBundle.loadString('assets/SoilCrops.csv');
  ...
  Future<void> showPredictionDetails(String label) async {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Soil & Plant Details'),
          content: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Soil: $label', style: TextStyle(fontSize: 18)),
              SizedBox(height: 10),
              Text('Plants Grown:', style: TextStyle(fontWeight: FontWeight.bold)),
